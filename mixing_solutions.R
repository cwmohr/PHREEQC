...
#### Conversion factors #### (not used yet)

NO3toN <- 14/(14+16*3) # from NO3 mass to N mass
NH4toN <- 14/(14+1*4)  # from NH4 mass to N mass
SO4toS <- 32/(32+16*4) # from SO4 mass to S mass
...
tot.species.output.df <- species.output.lst[[1]]
species.output.df <- species.output.lst[[2]]
...
for(i in seq_along(elmt.2char)){
  tmp.species <- gsub(elmt.2char[i], "", tmp.species)
}
...
#### Shiny selection instructions ####

## Select elements from list provided in `total.list` (check box)
## Examaple "Al" & "Fe"


## Species

## New list of species should be provided based on element selections.
species_available <- species.output.df %>% 
  filter(Al | Fe) %>% ## Example where both Al and Fe species
  select(Species) %>% 
  unique.data.frame() %>% 
  as_vector()

## Selection of which species  
species_selction <- sample(species_available, floor(0.5*length(species_available)), replace = FALSE) ## Here one should have an option of which species one would like to see (check box solution). By default all species should be selected. Example here is random selection (50%). 


## Phases

## New list of phases should be provided based on element selections.
phases_available <- phase.output.df %>% 
  filter(Al | Fe) %>% ## Example where both Al and Fe species
  select(Phase) %>% 
  unique.data.frame() %>% 
  as_vector()
...
## Selection of which species  
phase_selction <- sample(phases_available, floor(0.5*length(phases_available)), replace = FALSE) ## Here one should have an option of which phase one would like to see (check box solution). By default all phase should be selected. Example here is random selection (50%).
...
species.output.df %>% 
  filter(grepl("mix", sim_name), 
         Species %in% species_selction) %>% 
  ggplot(aes(x = sol2_frac, y = Molality, color = Species))+
  geom_line()+
  geom_point()+
  theme_bw()
...
species.output.df %>% 
  filter(grepl("mix", sim_name), 
         Species %in% species_selction) %>% 
  ggplot(aes(x = pH, y = Molality, color = Species))+
  geom_line()+
  geom_point()+
  theme_bw()
...
phase.output.df %>% 
  filter(grepl("mix", sim_name), 
         Phase %in% phase_selction) %>% 
  ggplot(aes(x = sol2_frac, y = SI, color = Phase))+
  geom_line()+
  geom_point()+
  theme_bw()
